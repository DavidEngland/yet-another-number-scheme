    ...
    def from_recurrence(initial_terms: List[Any], coefficients: List[Any], n: int) -> ExactSequence:
        ...
        for i in range(k, n):
            ...
            for j in range(k):
                ...
                term_val = term.to_float()
                coeff_val = coeff.to_float()
                
                # Add to next term
                next_val = next_term.to_float() + term_val * coeff_val
                next_term = ExactNumber(next_val)
...
if __name__ == "__main__":
    # Example 1: Create exact numbers
    a = ExactNumber(42)
    b = ExactNumber(Fraction(22, 7))  # Approximation of π
    c = ExactNumber("sqrt(2)")        # Algebraic number
    ...
    # Example 2: Generate and analyze Fibonacci sequence
    print("\nFibonacci sequence analysis:")
    fib = SequenceGenerator.fibonacci(10)
    patterns = fib.detect_patterns()
    print(patterns)
    
    # Example 3: Generate and analyze Bernoulli numbers
    print("\nBernoulli numbers analysis:")
    bernoulli = SequenceGenerator.bernoulli(10)
    for i, b in enumerate(bernoulli.elements):
        print(f"B_{i} = {b}")
    
    # Example 4: Find formula for a sequence
    print("\nPolynomial fitting:")
    seq = ExactSequence([0, 1, 4, 9, 16, 25])  # Square numbers
    formula = PatternDetector.polynomial_fit([x.to_float() for x in seq.elements])
    print(formula)
    
    # Example 5: Save and load a sequence
    seq.save("square_numbers.json")
    loaded_seq = ExactSequence.load("square_numbers.json")
    print("\nLoaded sequence:", [str(x) for x in loaded_seq.elements])
